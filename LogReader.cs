...
namespace FallGuysRecord_WPF_Framework
{
    internal class LogReader
    {
        ...
        public void Start()
        {
            ...
            timer.Interval = 10;
            reader = new Thread(ReadLog) { IsBackground = true };
            reader.Start();
        }
        ...
        {
            while (reader == null || reader.ThreadState == ThreadState.Unstarted)
            {
                await Task.Delay(50);
            }
            await Task.Factory.StartNew(() => reader?.Join());
        }
        ...
        private void parseLine(String line)
        {
            ...
            if (line.IndexOf("Client address: ", StringComparison.OrdinalIgnoreCase) > 0)
